...
<div class="max-w-lg w-full">
	<div class="flex items-center justify-between gap-4 mb-6">
		...
		<ButtonGroup.Root>
			...
			{#if data.event.isPublished}
				<Button variant="outline" size="sm" href="/events/{data.event.slug}/register"
					>Preview Form</Button
				>
			{/if}
		</ButtonGroup.Root>
	...
	<Card.Root class="mb-6">
		<Card.Header>
			...
			<Card.Description
				>Form parts are displayed as separate steps/sections in the registration form</Card.Description
			>
		</Card.Header>
		<Card.Content>
			<form method="POST" action="?/addPart" use:enhance class="flex gap-4 items-end">
				...
				<div class="flex-1 space-y-2">
					...
					<Input
						...
						name="title"
						required
						placeholder="e.g. Personal Information, Preferences..."
					/>
				</div>
				<div class="flex-1 space-y-2">
					...
					<Card.Header>
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-2 flex-1">
								<GripVerticalIcon
									class="w-5 h-5 text-muted-foreground cursor-grab active:cursor-grabbing"
								/>
								<div class="flex-1">
									<Card.Title class="flex items-center gap-2">
										...
										{#if part.conditionType !== 'none'}
											<span
												class="text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full"
											>
												Conditional
											</span>
										{/if}
									...
									{#if part.conditionType === 'field_value'}
										<div class="grid grid-cols-3 gap-4">
											<div class="space-y-2">
												...
												>
													{#each data.allFields ?? [] as field}
														<option
															value={field.id}
															selected={part.conditionConfig &&
																'fieldId' in part.conditionConfig &&
																part.conditionConfig.fieldId === field.id}
														>
															{field.label}
														</option>
													{/each}
											...
											<div class="space-y-2">
												...
												<Input
													name="conditionValue"
													value={part.conditionConfig && 'value' in part.conditionConfig
														? String(part.conditionConfig.value)
														: ''}
												/>
											</div>
										</div>
										...
										<div class="grid grid-cols-2 gap-4">
											<div class="space-y-2">
												...
												<Input
													name="conditionAfter"
													type="datetime-local"
													value={part.conditionConfig && 'after' in part.conditionConfig
														? String(part.conditionConfig.after ?? '')
														: ''}
												/>
											</div>
											<div class="space-y-2">
												...
												<Input
													...
													type="datetime-local"
													value={part.conditionConfig && 'before' in part.conditionConfig
														? String(part.conditionConfig.before ?? '')
														: ''}
												/>
											</div>
										</div>
								...
								>
									<div class="flex items-center justify-between">
										<div class="flex items-center gap-3 flex-1">
											...
											/>
											<span
												class="text-xs text-muted-foreground font-mono bg-muted px-2 py-0.5 rounded"
												>{field.type}</span
											>
											...
											<div class="space-y-3">
												...
												>
													...
													<input
														type="hidden"
														name="sortOrder"
														value={String(field.options?.length ?? 0)}
													/>
