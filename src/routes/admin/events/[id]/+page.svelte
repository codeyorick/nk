...
<div class="max-w-2xl">
  <div class="flex items-center justify-between mb-6">
    ...
    <div class="flex gap-2">
      <Button.Root variant="outline" href="/admin/events/{data.event.id}/form">Edit Form</Button.Root>
      <Button.Root variant="outline" href="/admin/events/{data.event.id}/registrations">Registrations</Button.Root>
    </div>
  </div>

  {#if form?.success}
    <div class="mb-4 rounded-md bg-green-50 dark:bg-green-900/20 p-4 text-green-700 dark:text-green-300">
    ...
    <Card.Root>
      ...
      <Card.Content class="space-y-4">
        ...
        <div class="space-y-2">
          ...
          <Textarea id="description" name="description" value={form?.values?.description ?? data.event.description ?? ""} />
        </div>

        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-2">
            <Label for="startDate">Start Date</Label>
            <Input id="startDate" name="startDate" type="datetime-local" value={form?.values?.startDate ?? formatDateForInput(data.event.startDate)} />
          </div>
          <div class="space-y-2">
            <Label for="endDate">End Date</Label>
            <Input id="endDate" name="endDate" type="datetime-local" value={form?.values?.endDate ?? formatDateForInput(data.event.endDate)} />
          </div>
        </div>
    ...
    <Card.Root class="border-destructive">
      ...
      <Card.Content>
        <p class="text-sm text-muted-foreground mb-4">
          Deleting this event will remove all registration forms, registrations, and associated data. This action cannot be undone.
        </p>
