<script lang="ts">
	...
	function isPartVisible(part: (typeof data.parts)[number]): boolean {
		...
		if (part.conditionType === 'date_range') {
			...
			if ('before' in config && config.before && now > new Date(config.before as string))
				return false;
			return true;
		}
		...
		<Card.Root>
			...
			<Card.Content>
				<p class="text-muted-foreground">
					Thank you for registering for <strong>{data.event.name}</strong>. We've sent a
					confirmation email. Please check your inbox and click the confirmation link to complete
					your registration.
				</p>
			</Card.Content>
			<Card.Footer>
		...
		{:else if isLastStep}
			...
			<form method="POST" use:enhance>
				...
				{#each Object.entries(fieldValues) as [fieldId, value]}
					<input type="hidden" name="field_{fieldId}" {value} />
				{/each}
							...
							<div class="space-y-2">
								...
								{:else if field.type === 'textarea'}
									<Textarea
										...
										value={fieldValues[field.id] ?? field.defaultValue ?? ''}
										oninput={(e) => {
											fieldValues[field.id] = (e.target as HTMLTextAreaElement).value;
										}}
									/>
									...
									<div class="flex items-center gap-2">
										<Checkbox
											...
											onCheckedChange={(checked) => {
												fieldValues[field.id] = String(checked);
											}}
										/>
									</div>
								{:else if field.type === 'select'}
									<select
										...
										value={fieldValues[field.id] ?? field.defaultValue ?? ''}
										onchange={(e) => {
											fieldValues[field.id] = (e.target as HTMLSelectElement).value;
										}}
									>
